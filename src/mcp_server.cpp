...
int findSubscription(uint8_t clientId, const String& uri) {
    for (int i = 0; i < subscriptionCount; i++) {
        if (subscriptions[i].clientId == clientId && subscriptions[i].uri == uri && subscriptions[i].active) {
            return i;
        }
    }
    return -1;
...
void addSubscription(uint8_t clientId, const String& uri) {
    ...
    if (slotIndex >= 0) {
        ...
        subscriptions[slotIndex].uri = uri;
...
void removeSubscription(uint8_t clientId, const String& uri) {
    int index = findSubscription(clientId, uri);
    if (index >= 0) {
...
void checkSubscriptions() {
    ...
    for (int i = 0; i < subscriptionCount; i++) {
        ...
        for (int j = 0; j < resourceCount; j++) {
            if (resources[j].uri == subscriptions[i].uri) {
...
void setupMcpServer() {
    ...
    Serial.println("MCP server started on port 9000");
    Serial.println("Connect to ws://" + WiFi.localIP().toString() + ":9000");
}
